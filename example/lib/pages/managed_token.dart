...
class _ManagedTokenState extends State<ManagedToken> {
              ...
              SliverFixedExtentList(
                delegate: SliverChildBuilderDelegate(
                    (context1, index) => GestureDetector(
                          ...
                          onTap: () async {
                            final txCall = MoveCallTransaction(packageId!, transactionModule!,
                                "burn", [], [objectId!, suiObjects![index].objectId], 100000000);

                            final burnResp = await widget.client.executeMoveCall(txCall);
                            if (burnResp.confirmedLocalExecution == true) {
                              print(burnResp.digest);
          ...
          ElevatedButton(
              onPressed: () async {
                final pulishTx = PublishTransaction(
                    ContractBytes.modules, ContractBytes.dependencies, 100000000);
                ...
                final treasuryCapObj = objectChanges.firstWhere((e) =>
                    e["type"] == "created" &&
                    e["objectType"].toString().contains("coin::TreasuryCap"));
        ...
        if (packageId != null)
          ElevatedButton(
              onPressed: () async {
                ...
                /// mint coin
                final callTx = MoveCallTransaction(packageId!, transactionModule!, "mint", [],
                    [objectId!, amount, widget.client.getAddress()], 100000000);
                ...
                if (resp.confirmedLocalExecution == true) {
                  ...
                  final balance = await widget.client
                      .getBalance(widget.client.getAddress(), coinType: coinType!);
              ...
              style: ElevatedButton.styleFrom(
                  minimumSize: const Size.fromHeight(50), textStyle: const TextStyle(fontSize: 18)),
  ...
  Future<List<SuiObject>> queryCoinObjects() async {
    ...
    final coinObjects = objectsResp.data
        .where((obj) => obj.data?.type == "0x2::coin::Coin<$coinType>")
        .map((x) => x.data!)
        .toList();
    return coinObjects;
  }
